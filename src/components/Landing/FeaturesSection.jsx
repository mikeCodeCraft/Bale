    ...
    <div className="max-w-5xl mx-auto px-2 sm:px-4 lg:px-6"> {/* Reduced max width and padding */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-7"> {/* Reduced gap */}
        {features.map((feature, index) => (
          <motion.div
            key={index}
            className="group relative overflow-hidden rounded-xl bg-white/5 border border-red-800 hover:border-red-600 transition-all duration-500 p-4" // Smaller border radius and padding
            initial={{ opacity: 0, y: 60 }}
            whileInView={{ opacity: 1, y: 0 }}
